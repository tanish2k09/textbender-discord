...
function isAdmin(userID) {
	return (userID == ids.quantumID) || (userID = ids.briID);
}

function isDev(userID) {
	return (userID == ids.quantumID);
}

function isSelf(userID) {
	return (userID == ids.selfID);
}

function isWhitelisted(userID) {
	return (userID == ids.bebeID || isAdmin(userID));
}
...
bot.on('message', function (user, userID, channelID, message, evt) {
	...
	if (message.substring(0, 1) == ';') {
		...
		// Bruh wyd, no command?
		if (cmd == null) {
			msgOp.nonsense(bot, channelID, isWhitelisted(userID));
			return;
		}

		// Blacklist retrack if annoying, handle that here
		if (blacklistRetrack && userID == ids.retracksID) {
		...
		switch(cmd) {
			...
			case 'ogreattreegivewisdom':
				if (userID == ids.bebeID)
					msgOp.post(bot, channelID, "My wisdom is only pure and simple: <@!" + ids.retracksID + "> is gay primus OG");
				else
			...
			case 'mock':
				...
				if (spongeBobString == null)
					return msgOp.nonsense(bot, channelID, isWhitelisted(userID));
