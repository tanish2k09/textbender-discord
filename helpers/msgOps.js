...
function getCommandDescriptions2() {
	...
	desc += "\n\nBot built by <@!" + ids.quantumID + ">";
	return desc;
}

module.exports = {
	nonsense: function (bot, ID, polite) {

		if (polite)
			msg = 'UwU daddy-san I\'m sorry I can\'t quite understand';
		else
			msg = 'Hey you little shit, use ;helpme';
	...
	postNice: function (bot, userID) {
		this.post(bot, ids.niceChannelID, "Nice - <@!" + userID + '>');
	},
	...
	postQuote: function (bot, message, userID) {
		...
		this.post(bot, ids.quoteChannelID, '"' + quote + "\"\n- <@!" + userID + ">");
	},

	postBruh: function (bot, userID) {
		this.post(bot, ids.bruhChannel, "*Ladies and gentlemen, a* ***bruh moment*** *has been declared by <@" + userID + ">*");
	},
